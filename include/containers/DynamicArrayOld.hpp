...
public:
    ...
    DynamicArrayOld() noexcept = default;

    explicit DynamicArrayOld(const Allocator& a) noexcept
        : m_allocator(a) { }

    explicit DynamicArrayOld(size_type sz, const Allocator& a = Allocator())
        : m_allocator(a) {
        if (sz == 0) [[unlikely]] {
            return;
        }
    ...
    DynamicArrayOld(const DynamicArrayOld& other)
        : DynamicArrayOld(
            other,
            AT::select_on_container_copy_construction(other.m_allocator)) { }

    DynamicArrayOld(const DynamicArrayOld& other, const Allocator& a)
        : m_allocator(a) {
    ...
    DynamicArrayOld(DynamicArrayOld&& other) noexcept
    ...
    DynamicArrayOld(DynamicArrayOld&& other, const Allocator& a) noexcept
        : m_allocator(a)
        , m_first(other.m_first)
        , m_last(other.m_last)
        , m_capacity(other.m_capacity) {
    ...
    DynamicArrayOld(
        std::initializer_list<T> il, const Allocator& a = Allocator())
        : DynamicArrayOld(il.begin(), il.end(), a) { }

    template <std::input_iterator It>
    DynamicArrayOld(It first, It last, const Allocator& a = Allocator())
        : m_allocator(a) {
        assign_range(first, last);
    }

    DynamicArrayOld& operator=(const DynamicArrayOld& other) {
        if (*this == other) [[unlikely]] {
            return *this;
        }

        DynamicArrayOld temp(other);
        swap(temp);
        return *this;
    }

    DynamicArrayOld& operator=(DynamicArrayOld&& other) noexcept(
    ...
    bool operator==(const DynamicArrayOld& other) const {
...
public:
    ...
    void swap(DynamicArrayOld& other) { impl_swap(other); }
...
private:
    ...
    inline void impl_swap(DynamicArrayOld& other) noexcept {
